...
const opito = async (req) => {
  const { badge, full_name, certificate, user, items, issued, expiry, course } =
    req.body

  const { expiry_type, name: courseName } = course

  const backgroundImage = './templates/certificates/certificate_opito.jpg'
  const logo = './templates/common/OPITO.jpg'
  ...
  if (logo) {
    doc.image(logo, 230, 695, { scale: 0.8 })
  }
...
module.exports = { standard, nimasa, opito }
